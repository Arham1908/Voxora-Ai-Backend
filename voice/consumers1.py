...
class VoiceAgentConsumer(AsyncWebsocketConsumer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # FIX 2 — Silence Watchdog state
        self._last_audio_time = time.monotonic()
        self._stream_ended = False
        self._watchdog_task = None
        
        # FIX 3 — Interruption state
        self._is_interrupted = False
        
        # FIX 4 — Session Resumption state
        self._retry_count = 0
        self._max_retries = 3
    ...
    async def connect(self):
        ...
        task = asyncio.create_task(self._session_management_loop())
        self._tasks.append(task)
        
        # FIX 2 — Start Silence Watchdog
    ...
    # ------------------------------------------------------------------
    # FIX 2 — Silence Watchdog Loop
    # ------------------------------------------------------------------
    async def _watchdog_loop(self):
        """Checks for silence every 0.5s and sends audio_stream_end if >1.5s."""
        try:
            while not self._disconnecting:
                await asyncio.sleep(0.5)
                silence_duration = time.monotonic() - self._last_audio_time
                
                if silence_duration > 1.5 and not self._stream_ended:
                    if self.gemini_session:
                        logger.debug(f"[Watchdog] Silence detected ({silence_duration:.1f}s), ending stream.")
    ...
    # ------------------------------------------------------------------
    # FIX 4 — Session Management Loop (Retry & Resumption)
    # ------------------------------------------------------------------
    async def _session_management_loop(self):
        while not self._disconnecting and self._retry_count < self._max_retries:
            try:
                await self._run_gemini_session()
            except Exception as e:
                self._retry_count += 1
                logger.error(f"[Session Exception] Retry {self._retry_count}/{self._max_retries}: {e}")
                if self._retry_count < self._max_retries:
                    await asyncio.sleep(1) # Backoff
                else:
                    logger.critical("Max retries reached. Closing session.")
    ...
    async def _run_gemini_session(self):
        ...
        live_config = types.LiveConnectConfig(
            ...
            # FIX 1 — Thinking Config
            thinking_config=types.ThinkingConfig(
                thinking_level="low",  # Proper reasoning for voice agent stability
            ),
                ...
                async for response in session.receive():
                    ...
                    if usage:
                        ...
                        if not getattr(self, "_logged_modality_sample", False) and self._usage_metrics["total"] > 0:
                            ...
                            # FIX 5 — Billing Log Update
                            m = self._usage_metrics
                            logger.info(
                                f"[WS DEBUG] Gemini 3.1 Session Token Usage updated: "
                                f"TI:{m['input_text']} AI:{m['input_audio']} TO:{m['output_text']} TH:{m.get('thinking', 0)} AO:{m['output_audio']}"
                            )
                    ...
                    if sc:
                        ...
                        if getattr(sc, "interrupted", False):
                            logger.info("[WS] Interrupted! Aborting current audio forwarding.")
                            self._is_interrupted = True
                            self._current_agent_turn = ""  # Clear partial text
                            continue

                        if getattr(sc, "turn_complete", False):
                            self._is_interrupted = False # Reset on completion
                    ...
                    if getattr(sc, "model_turn", None):
                        for part in sc.model_turn.parts:
                            ...
                            if inline and inline.data:
                                ...
                                # FIX 3 — Skip audio if interrupted
    ...
    async def _save_session_cost(self):
        ...
        if self._usage_metrics["total"] > 0 or duration > 0:
            try:
                # Gemini 3.1 Flash pricing (exact Task 2 rates):
                # Input:  $0.75/1M text, $3.00/1M audio
                # Output: $4.50/1M text, $4.50/1M thinking, $12.00/1M audio
                ...
                thinking_cost     = float(self._usage_metrics.get("thinking", 0)) * 0.0000045
                output_audio_cost = float(self._usage_metrics["output_audio"]) * 0.000012
                total_cost = input_text_cost + input_audio_cost + output_text_cost + thinking_cost + output_audio_cost
